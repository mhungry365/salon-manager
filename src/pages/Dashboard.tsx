...
export default function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    const cashInTill = cash + cashTips
    return { totalRevenue, totalTips, totalExpenses, cashInTill, revolut, twPrepaid, netProfit: totalRevenue - totalExpenses }
  }, [revenue, expenses])

  const paymentBreakdown = useMemo(() => {
    const map: Record<string, number> = {}
    revenue.forEach(r => {
      map[r.payment] = (map[r.payment] || 0) + Number(r.amount) + Number(r.tip || 0)
    })
  ...
  const sourceBreakdown = useMemo(() => {
    const map: Record<string, number> = {}
    revenue.forEach(r => {
      map[r.source] = (map[r.source] || 0) + Number(r.amount)
    })
    return Object.entries(map).map(([key, value]) => ({
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
              {paymentBoxes.map(({ key, label, color }) => {
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {revenue.length === 0 ? (
              <p className="text-gray-400 text-sm text-center py-8">No sales logged for this period</p>
            ) : (
