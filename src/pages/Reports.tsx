...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: rev }, { data: exp }] = await Promise.all([
        supabase.from('revenue_entries').select('*').eq('user_id', user.id).gte('date', from).lte('date', to),
        supabase.from('expense_entries').select('*').eq('user_id', user.id).gte('date', from).lte('date', to),
      ])
  ...
  const paymentBreakdown = useMemo(() => {
    ...
    revenue.forEach(r => {
      ...
      map[r.payment].count++
      map[r.payment].service += Number(r.amount)
      map[r.payment].tips += Number(r.tip || 0)
    })
    const total = revenue.reduce((s, r) => s + Number(r.amount) + Number(r.tip || 0), 0)
    return Object.entries(map).map(([key, v]) => ({
      key, label: PAYMENT_LABELS[key] || key,
      count: v.count, service: v.service, tips: v.tips,
      total: v.service + v.tips,
      pct: total > 0 ? ((v.service + v.tips) / total * 100).toFixed(1) : '0.0',
    }))
  }, [revenue])
  ...
  const sourceBreakdown = useMemo(() => {
    ...
    revenue.forEach(r => {
      ...
      map[r.source].count++
      map[r.source].revenue += Number(r.amount)
    })
    return Object.entries(map).map(([key, v]) => ({
      key, label: SOURCE_LABELS[key] || key, count: v.count, revenue: v.revenue,
    }))
  }, [revenue])
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      {['Method', 'Count', 'Service €', 'Tips €', 'Total €', '% Share'].map(h => (
                        <th key={h} className="text-left py-2 pr-4 text-xs font-semibold text-gray-600 uppercase tracking-wide">{h}</th>
                      ))}
                    </tr>
                  </thead>
                      ...
                      <tr key={p.key} className="hover:bg-gray-50">
                        <td className="py-2.5 pr-4">
                          <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                            p.key === 'cash' ? 'bg-green-100 text-green-700' :
                            p.key === 'revolut' ? 'bg-purple-100 text-purple-700' :
                            p.key === 'tw_prepaid' ? 'bg-blue-100 text-blue-700' :
                ...
                <tbody className="divide-y divide-gray-100">
                  {sourceBreakdown.map(s => (
                    <tr key={s.key} className="hover:bg-gray-50">
                      <td className="py-2.5 pr-4">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                          s.key === 'walkin' ? 'bg-teal-100 text-teal-700' :
                          s.key === 'treatwell' ? 'bg-purple-100 text-purple-700' :
